...
if ! grep -q '^  deploymentMode: SingleBinary$' gitops/apps/loki/values.yaml || \
   ...
   ! grep -q 'instance_enable_ipv6: true' gitops/apps/loki/values.yaml; then
  printf 'expected Loki SingleBinary mode, S3 schema, and IPv6 ring configuration\n' >&2
  exit 1
fi
...
if ! grep -q '^  upgradeCRDs: false$' gitops/apps/velero/values.yaml; then
  printf 'expected Velero CRD upgrade hook to be disabled for the chart image\n' >&2
  exit 1
fi
